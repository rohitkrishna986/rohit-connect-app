...
const MessageContainer = () => {
  ...
  useEffect(() => {
    console.log("Updated messages:", selectedChatMessages);
  ...
  const checkIfImage = (filePath) => {
    const imageRegex =
      /\.(jpg|jpeg|png|gif|bmp|tiff|tif|wepb|svg|ico|heic|heif)$/i;
    ...
    return (
      <div className={`${isSender ? "text-left" : "text-right"}`}>
        {message.messageType === "text" && (
          <div
            className={`border inline-block py-2 px-4 rounded-full my-2 max-w-[50%] break-words ${messageClasses}`}
          >
            {message.content}
          </div>
        )}
        {message.messageType === "file" && (
          <div
            className={`border inline-block p-4 rounded my-2 max-w-[50%] break-words ${messageClasses}`}
          >
  ...
  const renderChannelMessages = (message) => {
    return (
      ...
      >
        {message.messageType === "text" && (
          <div
            className={`border inline-block py-2 px-4 rounded-full my-2 max-w-[50%] break-words ${
        ...
        {message.messageType === "file" && (
          <div
            className={`border inline-block p-4 rounded my-2 max-w-[50%] break-words ${
              message.sender._id === userInfo.id
                ? "bg-[#8417ff]/5 text-[#8417ff]/90 border-[#8417ff]/50"
                : "bg-[#2a2b33]/5 text-white/80 border-[#ffffff]/20"
            }`}
        ...
        {message.sender._id !== userInfo.id ? (
          <div className="flex items-center justify-start gap-2">
            <Avatar className="h-5 w-5 rounded-full overflow-hidden">
              ...
              <AvatarFallback
                className={`uppercase h-5 w-5 text-lg flex items-center justify-center rounded-full ${getColor(
            ...
            <span className="text-[12px] text-white/60">{`${message.sender.firstName} ${message.sender.lastName}`}</span>
